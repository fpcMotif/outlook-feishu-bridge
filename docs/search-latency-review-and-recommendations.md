...
1. `convex/feishu/customersMirror.ts` + `src/hooks/useCustomerSearchServerIndex.ts`
   ...
   - Customer auto-match and server-index `matchEmail` only trigger mirror match/refresh when the current client email has a valid domain, and coalesce repeated local-part edits for the same domain across both remote work and local directory scans.
