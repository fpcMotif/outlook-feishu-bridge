...
// Shared post-token-response sequence for both the auth-code exchange and the
// refresh path: derive expiresAt with the early-expiry skew, fetch user info
// (the OIDC token endpoint doesn't return it), and persist the row. Returns the
// new access token.
async function persistTokenResponse(
  ctx: ActionCtx,
  sessionId: string,
  data: FeishuUserTokenResponse["data"],
): Promise<string> {
  const { access_token, refresh_token, token_type, expires_in } = data;
  const expiresAt = Date.now() + expires_in * 1000 - TOKEN_EXPIRY_SKEW_MS;
...
export async function exchangeCodeForUserToken(
  ...
  await persistTokenResponse(ctx, sessionId, parsed.data);
}
...
async function refreshUserToken(
  ...
  const access_token = await persistTokenResponse(ctx, sessionId, parsed.data);
