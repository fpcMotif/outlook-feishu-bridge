...
- **Short-query guard.** The CustomerPicker, server-index search adapter, and cache-miss action do not call server/Feishu fallback for one-character queries. Local directory matches still render immediately, and server search starts at two characters to avoid broad, low-quality cache-miss traffic.
