...
**Mirror Event Sync**:
The real-time path that keeps the **Customer Mirror** fresh between weekly Refreshes ([ADR-0020](docs/adr/0020-customer-mirror-prune-and-event-sync.md)): a **Feishu Event Subscription** webhook (`POST /feishu/events`, [convex/http.ts](convex/http.ts)) for the single Bitable event `drive.file.bitable_record_changed_v1`. A `record_deleted` tombstones the mirror row instantly; a `record_added` / `record_edited` re-reads that one record and upserts it. Verifies the `X-Lark-Signature` and decrypts the Encrypt-Key envelope before acting. Complements — never replaces — the weekly **Mirror Refresh** + **Mirror Prune** (the completeness floor).
_Avoid_: "polling" (it is push, not poll), "the bot" (no chat bot is involved), the fabricated event name `bitable.ui.record.updated_v1` (the audit's invention — the real event is `drive.file.bitable_record_changed_v1`).
