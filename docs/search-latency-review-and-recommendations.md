...
1. `convex/feishu/customersMirror.ts` + `src/hooks/useCustomerSearchServerIndex.ts`
   ...
   - Customer auto-match only triggers mirror match/refresh when the current client email has a valid domain, and coalesces repeated local-part edits for the same domain.
