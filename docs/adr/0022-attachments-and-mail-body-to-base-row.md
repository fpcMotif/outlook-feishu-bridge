...
|---|---|---|---|---|
| 1 | *(consolidated note)* ⚠️ name TBD | **changed** (3→1) | `string` | Text (1) / long-text |
...
| 8 | *(mail body)* ⚠️ name TBD | **new** | `string` | Text / long-text |
| 9 | *(attachment)* ⚠️ name TBD | **new** | `[{ file_token }]` | **Attachment (17)** |
...
1. **Categories dropped entirely.** Quotation / Sample / R&D Support are retired as a typed set — the UI is already a single note box ([RequestCards.tsx](../../src/components/taskpane/RequestCards.tsx) renders only the primary card). One note → one `Request Note` column. The `REQUESTS` map + `NOTE_FIELD` 3-column routing are deleted.
2. **Live Base column names: assumed, behind config constants.** Use `Request Note`, `Email Body`, `Attachments`, centralized as named constants in [serviceRow.ts](../../convex/feishu/serviceRow.ts) and clearly flagged "must match live Base — confirm before deploy". A wrong/missing name still fails or silently drops the write, so **confirm against the live schema (e.g. `listFields`) before the first production write.**
