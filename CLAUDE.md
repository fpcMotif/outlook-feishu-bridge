...
> **Why this section is safe.** The block at the top of this file between `<!-- convex-ai-start -->` and `<!-- convex-ai-end -->` is **auto-managed** by `bunx convex ai-files update` — anything inside it is overwritten on update. This section is **hand-maintained and lives OUTSIDE those markers**, so updates never clobber it (run `bunx convex ai-files update` freely; it only rewrites the marker block). Full ruleset / source of truth: [`convex/_generated/ai/guidelines.md`](convex/_generated/ai/guidelines.md) — read it first when touching `convex/`.
...
- **Package commands: `bun` / `bunx` only — never `npm` / `npx`.** Any `npx convex …` hint (including ones printed by `ai-files`) → run `bunx convex …`.
