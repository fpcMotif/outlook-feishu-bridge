...
- **Smaller interactive cache-miss pages.** Full mirror sync still uses Feishu's documented max `page_size=500`, but `searchAndCacheMiss` uses `page_size=50`. The picker returns at most 50 records, so cache misses no longer pull and upsert 500 rows on an interactive keystroke.
