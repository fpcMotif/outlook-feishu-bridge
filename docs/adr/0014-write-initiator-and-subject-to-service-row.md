...
- **`创建人` is not the salesperson.** Bitable's auto `创建人` column captures whichever identity *created* the row — that's the tenant-bot identity (`bitable:app`), not the human. A separate `Sales` User field is required to record the actual Initiator.
- **Subject is already in flight to Convex.** Mirroring it to Bitable costs one extra field write and makes the row scannable without opening Outlook.
- **Body stays out of Bitable.** The ADR-0010 reasoning still applies — bodies are unbounded text, vary wildly in formatting, would bloat row size, and the salesperson's structured Notes (Quotation Note / Sample Note / R&D Support Note) plus the ≤500-char preview on the Email Record together carry the triage signal.
