...
> **Status: REVERTED (2026-06-15).** The Self-Forward feature was removed in full: the Convex `m365/*` actions (token + native `message: forward`), the `useSelfForward` SPA hook, all intake state/UI wiring (the "Note to myself" chip), the `m365-diag-*` scripts, and the `M365_*` Convex env secrets. The Microsoft Graph door this ADR opened is **closed again** — the add-in makes no Graph calls. **What survives, and is now owned by [ADR-0014](0014-write-initiator-and-subject-to-service-row.md), not this ADR:** the Base `Email Conversation ID` column still stores the original Outlook Mail Item `conversationId` (the mailbox-local join key), and `src/office/mailItem.ts` still converts the Office item id to a REST id for the Convex recovery record. The rest of this document is retained as the historical record of the removed feature.
