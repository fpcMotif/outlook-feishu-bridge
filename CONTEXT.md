...
The pass is split across a **Mirror Refresh engine** and its **Convex adapter** (extract-then-test seam, [ADR-0019](docs/adr/0019-extract-then-test-seam.md)): the engine (`customerMirrorSync.ts`, `runMirrorRefresh`) is the pure, port-injected state-machine — page-walk → completeness check → the all-or-nothing prune gate → watermark — unit-testable without Convex against an in-memory fake; the adapter (`customersMirror.ts`) registers `fullSync`/`kick`, holds the single-flight lease, and supplies the real `{ clock, fetchPage, applyPage, applyDevFixtures, tombstone, finish }` port. The prune safety gate lives **inside** the engine, so "never tombstone on an incomplete sync" is enforced in pure code, not in the effectful loop.
_Avoid_: "mirror sync" (ambiguous with cache-miss backfill, which is incremental, not a full pass); calling `customerMirrorSync.ts` a "helpers" file (it is the engine, not loose utilities).
