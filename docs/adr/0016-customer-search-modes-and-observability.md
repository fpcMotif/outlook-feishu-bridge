...
- **Short-query guard.** The CustomerPicker does not call server fallback for one-character queries. Local directory matches still render immediately, and server search starts at two characters to avoid broad, low-quality cache-miss traffic.
