...
- **Coalesced cumulative PUT ("partial insert").** Tokens mint in waves; a single debounced PUT-coordinator writes the latest cumulative `file_token` set (only one PUT in flight, re-fires if new tokens landed). Attachments appear in waves, conflict-free, and a fill that dies after 6/10 leaves 6 on the row for rearm to finish.
