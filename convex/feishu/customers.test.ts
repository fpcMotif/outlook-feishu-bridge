...
describe("mapFeishuItemToCustomer", () => {
  // ADR-0020 RC2: the dedup/identity key is the immutable API `record_id`, NOT
  // the user-facing "Record Id" column. Here they diverge, so this pins that the
...
// The human "Record Id" column is demoted to a DISPLAY-ONLY projection (ADR-0020
// RC2). The helper still reads it (with API fallback) so a surface can show the
// human value, but it must never be the identity key — these tests pin both that
// contract and the fact that it CAN diverge from the API id.
