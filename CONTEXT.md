...
**Attachment staging**:
The phase where selected **Attachment** bytes sit in **Convex Backend** File Storage before the **Deferred Base write** uploads them to Feishu Drive (`upload_all`) and mints `file_token`s for the row create; user uploads may enter staging while the salesperson is still on the intake form. The submit path only stages bytes and hands the storage ids to `syncRequest` as `attachmentSources` — the Drive `upload_all` runs server-side in the worker, off the submit critical path ([ADR-0022](docs/adr/0022-attachments-and-mail-body-to-base-row.md)).
_Avoid_: "upload" alone (overloaded with Convex storage POST, Drive `upload_all`, and the Sync button); assuming the Drive mint blocks the submit (it now runs in the **Deferred Base write**).
...
- **`syncRequest` wall-clock on the Convex dashboard** is not a single metric for "Base Sync done" — the submit path awaits only **Attachment staging** (the Convex storage POST) before enqueue, while the Drive `upload_all` + record create both run in the separate **Deferred Base write** scheduled action ([ADR-0018](docs/adr/0018-request-sync-outbox-and-reconcile.md), [ADR-0022](docs/adr/0022-attachments-and-mail-body-to-base-row.md)).
